        ...
        List {
            if viewModel.prosContrasList.isEmpty {
                ...
                ForEach(viewModel.prosContrasList, id: \.id) { item in
                    NavigationLink {
                        VistaProContra(uid: uid, emocion: viewModel.emociones.first(where: { $0.id == item.idEmocion }) ?? Emocion(nombre: "", descripcion: "", color: "", image: "", sonido: ""), prosContrasExistente: item)
                    } label: {
